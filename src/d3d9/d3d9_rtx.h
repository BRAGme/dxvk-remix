...
namespace dxvk {
  ...
  struct D3D9Rtx {
    ...
    RTX_OPTION("rtx", bool, flipFrontFace, false, "Treat game geometry as counter-clockwise wound instead of clockwise, inverting which side of a triangle Remix considers the front face.\n"
               "Remix normally detects an inverted winding by testing whether viewToProjection * worldToView mirrors (rtx_instance_manager.cpp), but that check cannot see a mirror that lives in the per-object transform - which is exactly the case for games using a fused world-view transform, or emulators whose projection flips Y (PPSSPP negates the Y column, reversing effective winding, while the cull mode it sets is the one correct for the flipped raster pipeline).\n"
               "Symptom this fixes: opaque surfaces turn black or vanish from one viewing side while alpha-blended geometry looks correct - blended draws are exempt from culling, so they mask the problem. Prefer this over rtx.enableCulling = False, which fixes the same symptom by disabling culling entirely at the cost of light leaks and performance.");
    RTX_OPTION("rtx", bool, alwaysCaptureTexcoords, false, "When enabled, texture coordinates are always taken from the captured vertex shader output rather than the raw input buffer, even when the input buffer has a valid format. Needed for emulators and other games whose vertex shader is what makes the UVs final - e.g. PPSSPP applies the PSP's uvscale/offset, the texture matrix, and through-mode texel scaling inside the shader, so the input UVs are pre-transform and sampling with them gives visibly wrong textures. Leave disabled for games that hand over final UVs in the vertex buffer.");
